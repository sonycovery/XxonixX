...
let handler = async (m, { conn, usedPrefix: _p, args, command, __dirname }) => {
  ...
  try {
    ...
    if (teks == '404') {
      const sections = [
          ...
          const listMessage = {
            ...
            text: "Daftar Menu Bot.",
            footer: "Silahkan tekan tombol \"M E N U\" untuk melihat sub-menu Bot.\nWhatsApp-Bot™ ini hanya semata hiburan saja.",
            buttonText: "M E N U",
            sections
        }
    ...
    await conn.sendButton(m.chat, text.trim(), watermark, await (await fetch(selectedimage)).buffer(), [['D A S H B O A R D', `${_p}menu`]],  m)
  }
  } catch (e) {
